...
# Add parent tests/ directory to path for test_low_latency import
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    buffer = deep_ep.Buffer.nixl_buffer(rank=rank, low_latency_mode=False, explicitly_destroy=True, group=group, tcp_store_group=tcp_store)
    ...
    if args.test_ll_compatibility:
        ...
        buffer = deep_ep.Buffer.nixl_buffer(rank=rank, low_latency_mode=True, explicitly_destroy=True, tcp_store_group=tcp_store)
        buffer.update_memory_buffers(num_ranks=num_ranks, num_experts_per_rank=ll_num_experts // num_ranks,
                                     num_nvl_bytes=0, num_rdma_bytes=num_rdma_bytes)
        buffer.connect_ranks([i for i in range(num_ranks) if i != rank])
        dist.barrier()
        test_low_latency.test_main(ll_num_tokens, ll_hidden, ll_num_experts, ll_num_topk,

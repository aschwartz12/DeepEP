...
dispatch(void* packed_recv_x, void* packed_recv_x_scales,
         ...
         bool round_scale, int phases, deep_ep::gpu_nixl_ctx nixl_ctx) {
...
void dispatch(void* packed_recv_x, void* packed_recv_x_scales,
              ...
              cudaStream_t stream, int phases, deep_ep::gpu_nixl_ctx nixl_ctx) {
...
combine(void* combined_x,
        ...
        int num_warp_groups, int num_warps_per_group,
        int phases, bool zero_copy, deep_ep::gpu_nixl_ctx nixl_ctx) {
...
void combine(void* combined_x,
             ...
             bool use_logfmt,
             void* workspace, int num_device_sms,
             cudaStream_t stream, int phases, bool zero_copy, deep_ep::gpu_nixl_ctx nixl_ctx) {
    constexpr int kNumMaxTopk = 11;
    const int num_warp_groups = ceil_div(num_experts, num_device_sms);
    const int num_warps_per_group = 32 / num_warp_groups;
...
__forceinline__ __device__ void barrier(deep_ep::gpu_nixl_ctx nixl_ctx, int* mask_buffer_ptr, int thread_id) {
    ...
    if (thread_id < nixl_ctx.max_num_ranks && thread_id != nixl_ctx.rank) {
        ...
        if (not is_rank_masked(mask_buffer_ptr, dst_rank)) {
            ...
            if (wait_recv_cost > NUM_TIMEOUT_CYCLES) {
                printf("Warning: DeepEP timeout for barrier, rank %d, thread %d, dst_rank %d, expected value %d, actual value %d\n",
...
template <int kNumThreads>
__global__ void barrier_kernel(deep_ep::gpu_nixl_ctx nixl_ctx, int* mask_buffer_ptr) {
    const auto thread_id = static_cast<int>(threadIdx.x);
    barrier<kNumThreads>(nixl_ctx, mask_buffer_ptr, thread_id);
}

void barrier(deep_ep::gpu_nixl_ctx nixl_ctx, int* mask_buffer_ptr, cudaStream_t stream) {
    constexpr int kNumThreads = 32;
...
template <int kNumThreads>
__global__ void clean_buffer_kernel(int* clean_0, int num_clean_int_0,
                                    int* clean_1, int num_clean_int_1,
                                    deep_ep::gpu_nixl_ctx nixl_ctx, int* mask_buffer_ptr) {
    const auto thread_id = static_cast<int>(threadIdx.x);
...
void clean_buffer(void* clean_0, int num_clean_int_0,
                  void* clean_1, int num_clean_int_1,
                  int* mask_buffer, deep_ep::gpu_nixl_ctx nixl_ctx,
                  cudaStream_t stream) {
    constexpr int kNumThreads = 256;

    SETUP_LAUNCH_CONFIG(1, kNumThreads, stream);
    LAUNCH_KERNEL(&cfg, clean_buffer_kernel<kNumThreads>,
                  reinterpret_cast<int*>(clean_0), num_clean_int_0,
                  reinterpret_cast<int*>(clean_1), num_clean_int_1,
                  nixl_ctx, mask_buffer);
}

...
#ifdef USE_NIXL
    const TransportCtx& transport = gpu_ctx;
#else
    const TransportCtx transport{internode::cpu_rdma_team, rank, num_ranks, nvl_rank, sync_buffer_ptr};
#endif
...
void Buffer::clean_low_latency_buffer(int num_max_dispatch_tokens_per_rank, int hidden, int num_experts) {
    ...
    LowLatencyLayout<tp_counter_t> layout(rdma_buffer_ptr, num_max_dispatch_tokens_per_rank, hidden, get_ll_num_ranks(), num_experts);
...
void Buffer::barrier() {
    auto compute_stream = at::cuda::getCurrentCUDAStream();
    internode_ll::barrier(gpu_ctx, mask_buffer_ptr, compute_stream);
}
#endif
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("get_low_latency_rdma_size_hint", &deep_ep::get_low_latency_rdma_size_hint<deep_ep::tp_counter_t>);
